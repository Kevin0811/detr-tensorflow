...
train_summary_writer = tf.summary.create_file_writer('logs/v3/resnet+mask+gesture+shared-' + dataset + '-' + current_time)
...
def validation(val_model, val_data, val_step):
    ...
    # 紀錄損失值 
    with train_summary_writer.as_default():
        tf.summary.scalar('val_loss', val_avg_loss/current_step, total_train_step)
    
    print('\r>>> Validation Compeleted')
    print(f"Results: average loss : [{val_avg_loss/current_step:.3f}], crd loss : [{val_crds_loss:.3f}], aux loss : [{val_aux_loss:.3f}], gesture loss : [{val_gesture_loss:.3f}]")
    return val_step
