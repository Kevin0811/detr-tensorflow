...
backbone_initial_lr = 0.000025
mask_initial_lr = 0.0001
shared_initial_lr = 0.00001
pos_initial_lr = 0.00025
gesture_initial_lr = 0.00001
...
for epoch_nb in range(training_epoch):
    ...
    for step , (images, skeleton_lable, mask, gesture_label) in enumerate(train_dt):
        ...
        # 取得權重
        backbone_weights = custom_model.get_layer("Backbone_layer").trainable_weights
        #mask_weights = custom_model.get_layer("Mask_layer").trainable_weights
        #pos_weights = custom_model.get_layer("Position_layer").trainable_weights
        gesture_weights = custom_model.get_layer("Gesture_layer").trainable_weights
        shared_weights = custom_model.get_layer("Shared_layer").trainable_weights
