...
from detr_tf.data.tfcsv import load_tfcsv_dataset, load_k4b_dataset, load_coco_dataset
...
def build_model(config):
    ...
    detr = get_detr_model(config, include_top=False, nb_class=None, num_decoder_layers=2, num_encoder_layers=2, weights=None)
...
def run_finetuning(config):
    ...
    train_dt = load_coco_dataset(config.batch_size, "train_annos.txt")
    valid_dt = load_coco_dataset(config.batch_size, "test_annos.txt")
    ...
    for epoch_nb in range(180):
        ...
        config.backbone_lr.assign(1e-5) # 0.001
        ...
        #tf.saved_model.save(detr, '/weights/coco_hand_weight')
        
        if epoch_nb % 5 == 0 and epoch_nb != 0:
            detr.save_weights("weights/COCO/detr-model_" + str(epoch_nb) + ".ckpt")
            #tf.saved_model.save(detr, '/weights/coco_hand_weight')

            # 驗證
            eval_image, lable = training.eval(detr, valid_dt, config, evaluation_step=250)
            show_result(eval_image, lable)
            # print last data coordination
            #for coords in lable:
                    #print("coordination : " + str(coords))
